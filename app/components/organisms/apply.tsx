...
import { appllicationInfoSchema } from "../validations/apply-info";
import { useForm } from "react-hook-form";
import { createRegistration } from "../../(website)/servers/applyformData";
import { zodResolver } from "@hookform/resolvers/zod";
import { ApplySchemaTypes } from "../../lib/interfaces";
import Notification from "../../lib/toast";
import { toast } from "react-toastify";
import Potentialsection from "./potential";

const ApplyNow: React.FC = () => {
  const notify = () => toast.success("Application submitted successfully");
  const failed = () => toast.error("Application Failed please try again");
  ...
  return (
    <div className="relative mt-20  h-screen w-full">
      <div className="flex h-2/6  w-full items-center md:h-4/6">
        <Image
          src={apply}
          alt="sectioning"
          ...
          priority
        />
        <div className=" absolute w-full  px-6 text-white lg:px-20">
          <h1
            className={` ${playfair.className} w-3/5 text-xl  mobile:max-md:text-3xl md:text-6xl`}
          >
            Apply Now
          </h1>
          <h2 className="mt-10 w-1/2 font-sans text-lg font-bold mobile:max-md:w-[100%]  md:text-2xl">
            Join us for an opportunity of a lifetime!
          </h2>
        </div>
      </div>

      <div className=" flex w-full justify-center mobile:max-md:mx-0">
        <form
          {...form}
          onSubmit={form.handleSubmit(onSubmit)}
          className=" my-20 flex w-full flex-col space-y-3 mobile:max-md:mx-5 mobile:max-md:my-5 mobile:max-md:space-y-0 md:w-8/12"
        >
          ...
          {/* all div  */}
          <div className="flex w-full flex-col justify-between py-8 md:flex-row">
            {/* div 3  */}
            <div className="flex w-full flex-col space-y-5 md:w-[45%]">
              <p className="text-xl font-semibold">
                Relationship with Guardian{" "}
              </p>
              <div className="mt-6 flex flex-col space-y-3  p-4 text-lg">
                <label className=" font-sans">
                  <input
                    {...register("gender")}
                    className=" bg-gray-200 py-4  pl-4"
            ...
            {/* div 4  */}
            <div className="flex w-full flex-col space-y-5 py-3 md:w-[45%]">
              <label className="text-xl font-semibold">If Other, specify</label>
              <input
              ...
              <label className="pt-0 text-xl font-semibold md:pt-20">
                Email of Guardian
              </label>
              <input
